...
import (
	...
	quicssh "github.com/francoismichel/quicssh"
	quicsshMessages "github.com/francoismichel/quicssh/message"
	"github.com/francoismichel/quicssh/unix_server"
	util "github.com/francoismichel/quicssh/util"
...
// var runningSessions = make(map[quicssh.Channel]*runningSession)
var runningSessions = util.NewSyncMap[quicssh.Channel, *runningSession]()
...
func forwardUDPInBackground(ctx context.Context, channel quicssh.Channel, conn *net.UDPConn) {
	go func() {
		defer conn.Close()
		for {
			select {
...
func execCmdInBackground(channel quicssh.Channel, openPty *openPty, user *unix_util.User, runningCommand *runningCommand, authAgentSocketPath string) error {
		...
		for {
			...
			case stdoutResult, ok := <-stdoutChan:
				if !ok {
					...
					_, err2 := channel.WriteData(buf, quicsshMessages.SSH_EXTENDED_DATA_NONE)
					if err2 != nil {
			...
			case stderrResult, ok := <-stderrChan:
				if !ok {
					...
					_, err2 := channel.WriteData(buf, quicsshMessages.SSH_EXTENDED_DATA_STDERR)
					if err2 != nil {
						log.Error().Msgf("could not write the pty's output in an SSH message: %+v\n", err)
						return
					}
			...
			if stdoutChan == nil && stderrChan == nil && execResultChan == nil {
				err := channel.SendRequest(&quicsshMessages.ChannelRequestMessage{
					WantReply:      false,
					ChannelRequest: &quicsshMessages.ExitStatusRequest{ExitStatus: execExitStatus},
				})
				if err != nil {
...
func newPtyReq(user *unix_util.User, channel quicssh.Channel, request quicsshMessages.PtyRequest, wantReply bool) error {
...
func newX11Req(user *unix_util.User, channel quicssh.Channel, request quicsshMessages.X11Request, wantReply bool) error {
	return fmt.Errorf("%T not implemented", request)
}

func newCommand(user *unix_util.User, channel quicssh.Channel, loginShell bool, command string, args ...string) error {
	var session *runningSession
	session, ok := runningSessions.Get(channel)
	if !ok {
		return fmt.Errorf("internal error: cannot find session for current channel")
...
func newShellReq(user *unix_util.User, channel quicssh.Channel, wantReply bool) error {
...
func newCommandInShellReq(user *unix_util.User, channel quicssh.Channel, wantReply bool, command string) error {
	return newCommand(user, channel, false, user.Shell, "-c", command)
}

func newSubsystemReq(user *unix_util.User, channel quicssh.Channel, request quicsshMessages.SubsystemRequest, wantReply bool) error {
	return fmt.Errorf("%T not implemented", request)
}

func newWindowChangeReq(user *unix_util.User, channel quicssh.Channel, request quicsshMessages.WindowChangeRequest, wantReply bool) error {
	return fmt.Errorf("%T not implemented", request)
}

func newSignalReq(user *unix_util.User, channel quicssh.Channel, request quicsshMessages.SignalRequest, wantReply bool) error {
...
func newExitStatusReq(user *unix_util.User, channel quicssh.Channel, request quicsshMessages.ExitStatusRequest, wantReply bool) error {
	return fmt.Errorf("%T not implemented", request)
}

func newExitSignalReq(user *unix_util.User, channel quicssh.Channel, request quicsshMessages.ExitSignalRequest, wantReply bool) error {
	return fmt.Errorf("%T not implemented", request)
}

func handleUDPForwardingChannel(ctx context.Context, user *unix_util.User, conv *quicssh.Conversation, channel *quicssh.UDPForwardingChannelImpl) error {
...
func handleTCPForwardingChannel(ctx context.Context, user *unix_util.User, conv *quicssh.Conversation, channel *quicssh.TCPForwardingChannelImpl) error {
	// TODO: currently, the rights for socket creation are not checked. The socket is opened with the process's uid and gid
	// Not sure how to handled that in go since we cannot temporarily change the uid/gid without potentially impacting every
	// other goroutine
...
func newDataReq(user *unix_util.User, channel quicssh.Channel, request quicsshMessages.DataOrExtendedDataMessage) error {
	runningSession, ok := runningSessions.Get(channel)
	if !ok {
		return fmt.Errorf("could not find running session for channel %d (conv %d)", channel.ChannelID(), channel.ConversationID())
	}
...
func handleAuthAgentSocketConn(conn net.Conn, conversation *quicssh.Conversation) {
...
func listenAndAcceptAuthSockets(cancel context.CancelCauseFunc, conversation *quicssh.Conversation, listener net.Listener, maxSSHPacketSize uint64) {
	defer cancel(nil)
...
func openAgentSocketAndForwardAgent(parent context.Context, conv *quicssh.Conversation, user *unix_util.User) (string, error) {
...
func main() {
	...
	urlPath := flag.String("url-path", "/quicssh-term", "the secret URL path on which the quicssh server listens")
	...
	if badCertificatesConf {
		...
		fmt.Fprintln(os.Stderr, "https://github.com/francoismichel/quicssh/blob/main/generate_openssl_selfsigned_certificate.sh")
		os.Exit(-1)
	...
	if *verbose {
		...
		logFileName := os.Getenv("QUICSSH_LOG_FILE")
		if logFileName == "" {
			logFileName = util.FullPathWithEnv("/var/log/quicssh.log", "PREFIX")
		}
	...
	go func() {
		...
		quicsshServer := quicssh.NewServer(30000, 10, &server, func(authenticatedUsername string, conv *quicssh.Conversation) error {
			...
			for {
				...
				case *quicssh.UDPForwardingChannelImpl:
					handleUDPForwardingChannel(conv.Context(), authenticatedUser, conv, c)
				case *quicssh.TCPForwardingChannelImpl:
		...
		quicsshHandler := quicsshServer.GetHTTPHandlerFunc(context.Background())
		handler, err := unix_server.HandleAuths(context.Background(), enablePasswordLogin, 30000, quicsshHandler)
		if err != nil {
