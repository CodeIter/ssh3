...
/*
 * In quicssh, authorized_keys are replaced by authorized_identities where a use can specify classical
...
func DefaultIdentitiesFileNames(user *unix_util.User) []string {
	return []string{path.Join(user.Dir, ".quicssh", "authorized_identities"), path.Join(user.Dir, ".ssh", "authorized_keys")}
}
...
func (i *PubKeyIdentity) Verify(genericCandidate interface{}, base64ConversationID string) bool {
	...
	case util.JWTTokenString:
		...
		if claims, ok := token.Claims.(jwt.MapClaims); ok {
			...
			if clientId, ok := claims["client_id"]; !ok || clientId != fmt.Sprintf("quicssh-%s", i.username) {
				return false
			}
